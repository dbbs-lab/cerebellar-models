...
from cerebellar_models.optimization.optimizer import ArchiveND, Constraint, Optimizer
...
data_folder = os.path.join(os.path.dirname(__file__), "tofit_eglif/results_tofitEglif/PC_Z-/")
...
def evaluate_PC(opt, ind, cell_params=None):
    ...
    nest = opt._extract_nest_features(targ_df=targ)
    ...
    gap = gap_loss(targ, nest, sel)
    adap = adaptation_index_loss(targ, nest, protocol, sel=sel)
    neg_loss = post_rebound_loss(targ, nest, protocol, thr=thr, sign="neg", window=150.0, scale=7)
    ...
    opt.add_constraint(
        Constraint(
            ...
            ctx={
                ...
                "idx_k2": opt.opt_params.index("k_2"),
                "C_m": cell_params["C_m"],
