...
def nest_single_sim(current, cell_params, protocol, nest_model="eglif_multirec_opt", multimeter=False):
    ...
    nest.SetStatus(gen, {"amplitude": current, "start": protocol["start_stim"], "stop": protocol["end_stim"]})
    sr = nest.Create("spike_recorder")

    mm = None
    if multimeter:
        mm = nest.Create("multimeter", params={"interval": 0.1, "record_from": ["V_m"], "record_to": "memory"})
...
def plot_results(multicomp_feat, results, cell_params, start_stim, stop_stim, cell_name, output_dir="."):
    from matplotlib.gridspec import GridSpec
    import matplotlib.ticker as mticker

    os.makedirs(output_dir, exist_ok=True)

    def add_fake_spikes_to_vm(t, v, spikes, v_peak=20.0, sigma_ms=0.35):
    ...
    def style_I_axis(axI, show_xlabel=False):
        ...
        axI.tick_params(axis="y", which="major", labelsize=7, length=3, width=1, colors="#7a1f2b", pad=3)
    ...
    for i, Iamp in enumerate(ordered_currents):
        ...
        axV.axvspan(start_stim, stop_stim, facecolor="#ffe680", alpha=0.18, edgecolor="none", zorder=-10)

        v_plot = add_fake_spikes_to_vm(t, v, spikes, v_peak=v_peak, sigma_ms=0.35)
        axV.plot(t, v_plot, linewidth=1.25, color="black", zorder=2)
        axV.axhline(V_th, color="gray", linestyle="--", linewidth=0.9, zorder=1)
    ...
    ax.plot(currents, multicomp_freqs, "o", label="NEURON (target)", color="gray",
            markersize=5, linestyle="--", linewidth=1.0)
    ax.plot(currents, nest_freqs, "s-", label="NEST (best)",
            color=[0.275, 0.800, 0.275, 1.0], markersize=5, linewidth=1.3)
...
def plot_error_bar(error_dict, cell_name, out_fig_dir):
    ...
    bars = ax.bar(x, vals, color=[0.275, 0.800, 0.275, 1.0], alpha=0.6)
