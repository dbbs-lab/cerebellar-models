...
def gene_to_params_SC(gene):
    ...
    if gene.size != 6:
        raise ValueError(f"SC gene must be length 6, got {gene.size}")

    # Mapping as in your SC code:
    # gene = [I_e, k_adap, k_2, A1, A2, k_1]
    return {
        "t_ref": 1.59,
        "C_m": 14.6,
        "V_th": -53,
...
if __name__ == "__main__":
    ...
    path_genes = "neuron_opt/SC_opt/pareto_snapshots/pareto_genes_150.npy"
    path_fit = "neuron_opt/SC_opt/pareto_snapshots/pareto_fitness_150.npy"
    path_hist = "neuron_opt/SC_opt/best_history.pkl"
    ...
    # -------------------------
    # 1) Admissible set based on stored Pareto fitness
    # -------------------------
    ...
    # -------------------------
    # 2) Tie-break with post-first-spike (evaluated)
    # -------------------------
    ...
    # -------------------------
    cell_params_best = gene_to_params_SC(best_gene)
    ...
    neg_loss = post_rebound_loss(targ, nest, protocol, thr=thr, sign="neg", window=70.0)
