...
def compute_metrics(spiketrains, nb_neurons, target_features, ct_names, order_ct):
    ...
    for nb, spikes, ct in zip(nb_neurons, spiketrains, ct_names):
        ...
        z_tol = np.abs((mean_fr - target_mean) / tol_mean)
        mean_fr_error = 0.0 if z_tol <= 1.0 else z_tol

        z_std = (std_fr - target_std) / tol_std
        std_error = max(0.0, abs(z_std) - 1.0)

        min_fr = 5.0
        max_silent_frac = 0.10  # max 10% silenti
        silent_frac = np.mean(all_fr < min_fr)
...
if __name__ == "__main__":
    ...
    transient = 300.0
    order_ct = ["stellate_cell"]
    bounds = {
        # "basket_to_basket": (0.01, 1.5),
        # "parallel_fiber_to_basket": (0.01, 1.5),
        "parallel_fiber_to_stellate": (0.01, 1.5),
        "stellate_to_stellate": (0.01, 1.5),
    }
    target_features = {
        #'basket_cell': {'mean_fr': 11., 'std': 5.1, 'tol_mean_fr': 3., 'std_tol': 2.},
        "stellate_cell": {"mean_fr": 9.4, "std": 12, "tol_mean_fr": 2.0, "std_tol": 2.0}
    }
    if comm.Get_rank() == 0:
        fitness = {
            # "Mean Firing Rate Basket": -1,
            # "Standard Deviation Basket": -1,
            "Mean Firing Rate Stellate": -1,
            "Standard Deviation Stellate": -1,
        }
