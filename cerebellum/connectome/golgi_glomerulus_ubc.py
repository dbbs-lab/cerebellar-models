...
class ConnectomeGolgiGlomerulusUBC(ConnectionStrategy):
    ...
    intermediate_connectivity_names = config.attr(type=list[str], required=True)
    ...
    def _connect_type(self, pre_ct, pre_ps, post_ct, post_ps):
        ...
        #Look for gloms less than 150 um from golgi
        chunk = pre_ps.get_loaded_chunks()[0]
        ...
        #pre_chunks = list(post_ps.get_loaded_chunks())
        chunks_ids = [c.id for c in selected_chunks]
        sorted_ids = np.argsort(chunks_ids)
        sorted_chunks = []
        for i in sorted_ids:
            sorted_chunks.append(selected_chunks[i])

        granule_connections = []
        granule_connections_branch_point = []
        glom_pos_list = []
        #Get the glom_to_granule connections
        for idx,connectivity_name in enumerate(self.intermediate_connectivity_names):
            cs = self.scaffold.get_connectivity_set(connectivity_name)
            ...
            glom_pos_list.append(self.intermediate.cell_types[idx].get_placement_set(chunks=sorted_chunks).load_positions()[unique_gloms_id])
            

        glom_pos = np.vstack(glom_pos_list)
        n_glomeruli = len(glom_pos)
        n_golgi = len(golgi_pos)

        post_locs = np.full((self.convergence*n_golgi*len(post_ps),3),-1,dtype=int)
        pre_locs = np.full((self.convergence*n_golgi*len(post_ps),3),-1,dtype=int)
