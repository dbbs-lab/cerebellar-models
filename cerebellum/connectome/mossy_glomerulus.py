...
@config.node
class ConnectomeMossyGlomerulus(InvertedRoI, ConnectionStrategy):
    ...
    def connect(self, pre, post):
        # We use a truncated exponential distribution to favour the MF closer the
        ...
        for pre_ps in pre.placement:
            for post_ps in post.placement:
                ...
                rolls = np.floor(len(mossy_pos) * exp_dist.rvs(size=n_glom)).astype(int)
