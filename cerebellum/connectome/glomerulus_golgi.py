...
class ConnectomeGlomerulusGolgi(ConnectionStrategy):
    ...
    def _connect_type(self, pre_ct, pre_ps, post_ct, post_ps):
        ...
        for i, golgi in enumerate(golgi_pos):
            ...
            tips_coordinates = np.full(
                (len(basal_dendrides_branches), 3), 0, dtype=float
            )
            ...
            for id_g, glom_p in enumerate(glomeruli_pos):
                pts_dist = np.sqrt(
                    np.power(tips_coordinates[:, 0] + golgi[0] - glom_p[0], 2)
                ...
                # distribution mapped through the distance indices: high chance to pick
                # closeby points.
                pt_idx = sorted_pts_ids[
                    int(
                        len(basal_dendrides_branches)
                        * rolls[np.random.randint(0, len(rolls))]
                    )
                ]

                # The id of the branch is the id of the terminal_branches plus the id of
                # the first dendritic branch
                post_locs[ptr + id_g, 1] = terminal_branches_ids[pt_idx]
                # We connect the tip of the branch
                post_locs[ptr + id_g, 2] = (
                    len(basal_dendrides_branches[pt_idx].points) - 1
                )
            ptr += connected_gloms
