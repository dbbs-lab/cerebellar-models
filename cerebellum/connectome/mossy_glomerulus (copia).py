...
class ConnectomeMossyGlomerulus(ConnectionStrategy):
    ...
    def _connect_type(self, pre_ct, pre_ps, post_ct, post_ps):
        ...
        granule_pos = (
            self.intermediate.cell_types[0].get_placement_set().load_positions()
        )
        ...
        for i, mossy in enumerate(mossy_pos):
            ...
            if num_candidates <= glomeruli_to_connect:
                ...
                post_locs[ptr : ptr + glomeruli_to_connect, 0] = final_candidates[0][
                    0:glomeruli_to_connect
                ]
        ...
        random_ids = np.random.randint(
            low=0, high=n_mossy, size=len(id_non_connected), dtype=int
        )
        print("random_ids")
        ...
        post_locs[0 : len(glomeruli_pos), 0] = np.arange(
            0, len(glomeruli_pos), dtype=int
        )
        print(np.arange(0, len(glomeruli_pos), dtype=int))
        self.connect_cells(
            pre_ps,
            post_ps,
            pre_locs[: len(glomeruli_pos)],
            post_locs[: len(glomeruli_pos)],
        )
