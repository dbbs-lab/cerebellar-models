...
class ConnectomeGlomerulusGranule(ConnectionStrategy):
    ...
    # In this way, given  if for each glomerulus in the ROI we select randomly the
    # mossyfiber to connect, the divergence ratio is automatically around 20.
    ...
    def _connect_type(self, pre_ct, pre_ps, post_ct, post_ps):
        ...
        for current in unique_mossy:
            glom_ids = np.where(
                (gid_local[:, 0] == current[0]) & (gid_local[:, 1] == current[1])
            )
