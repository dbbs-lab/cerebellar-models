...
class ConnectomeGolgiGlomerulusGranule(ConnectionStrategy):
    ...
    def _connect_type(self, pre_ct, pre_ps, post_ct, post_ps):
        ...
        print(
            "Number of gloms",
            len(
                self.intermediate.cell_types[0].get_placement_set(
                    chunks=pre_ps.get_loaded_chunks()
                )
            ),
        )
        print("ROI")
        ...
        for lchunk, itr in cs.nested_iter_connections(
            "out", local_=pre_ps.get_loaded_chunks()
        ):
            local_n_granule = 0
            ...
            for i in range(
                len(self.intermediate.cell_types[0].get_placement_set(chunks=[lchunk]))
            ):
            ...
            for gchunk, (local_locs, global_locs) in itr:
                ...
                # print("Chunk:",gchunk)
                local_n_glom = self.intermediate.cell_types[0].get_placement_set(
                    chunks=[gchunk]
                )
                ...
                for i, current in enumerate(unique_glomeruli):
                    ...
                    granule_connections[i + n_local_glom] = np.append(
                        granule_connections[i], temp[granule_ids[0]]
                    )
                    granule_connections_branch_point[i + n_local_glom] = np.append(
                        granule_connections_branch_point[i],
                        global_locs[granule_ids[0], 1:],
                        axis=0,
                    )
            for gl in unique_glomeruli:
                glom_ids_list.append(gl[0] + n_local_glom)
            n_local_glom += len(
                self.intermediate.cell_types[0].get_placement_set(chunks=[lchunk])
            )
            total_count += local_n_granule

        # print(granule_connections)

        num_gloms = len(granule_connections)

        glom_pos = (
            self.intermediate.cell_types[0]
            .get_placement_set(chunks=pre_ps.get_loaded_chunks())
            .load_positions()
        )
        ...
        branches_names = []
        for string in self.presynaptic.subcell_labels:
            branches_names.append(string)

        ptr = 0
        for id_golgi, golgi in enumerate(golgi_pos):

            # Load morphology
            morpho = morphologies[morpho_id[id_golgi]]
            axon_branches = morpho.get_branches(labels=branches_names)
            first_axon_branch_id = morpho.branches.index(axon_branches[0])
            ...
            num_glom_to_connect = np.min(
                [self.convergence, len(to_connect), len(granule_connections)]
            )
