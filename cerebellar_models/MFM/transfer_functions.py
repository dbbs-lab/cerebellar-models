...
def _connect_neuron(neuron, sim_info, rates_dict, rng):
    ...
    for conn_tag, info in sim_info["connections"].items():
        ...
        if sim_info.get("stochastic_convergence", False):
            ...
            n_syn = max(
                1, int(round(rng.normal(mean_conv, std_conv) if std_conv > 0 else mean_conv))
            )
        else:
...
class TransferFunction:
    ...
    def summary(self):
        ...
        def _fmt(idx):
            return ", ".join(
                f"{t}={self.freq_axes[i][idx[i]]:.1f}" for i, t in enumerate(self.tags)
            )
        ...
        print(
            f"Std mean: {self.tf_std.mean():.2f} Hz  |  max {self.tf_std.max():.2f} Hz @ ({_fmt(idx_max_std)})"
        )
    ...
    def _make_3d_scatter(self, data, cmap, colorbar_label, save_path=None):
        ...
        xt, xl = _ticks(ax0)
        ax3d.set_xticks(xt)
        ax3d.set_xticklabels(xl, fontsize=9)
        yt, yl = _ticks(ax1)
        ax3d.set_yticks(yt)
        ax3d.set_yticklabels(yl, fontsize=9)
        zt, zl = _ticks(ax2)
        ax3d.set_zticks(zt)
        ax3d.set_zticklabels(zl, fontsize=9)
...
class TransferFunctionSimulator:
    ...
    def _build_sim_info(self, yaml_path, tf_dict_path, cell_name):
        ...
        cell_params = circuit["simulations"]["nest_basal_activity"]["cell_models"][cell_name][
            "constants"
        ]
    ...
    def compute_tf(self, save=True):
        ...
        if self.n_workers == 1:
            with tqdm(
                total=len(all_indices), desc=self.cell_name, unit="sim", bar_format=bar_fmt
            ) as pbar:
