...
PARALLEL_SETTINGS = {
    ...
    "GOTO_NUM_THREADS":"1","VECLIB_MAXIMUM_THREADS":"1","OMP_PROC_BIND":"FALSE","OMP_WAIT_POLICY":"PASSIVE",
    "NEST_NUM_THREADS":"1","NEST_NO_BANNER":"1",
    "OMPI_MCA_pml":"ob1","OMPI_MCA_btl":"self,tcp","OMPI_MCA_btl_base_verbose":"0",
}
...
def _worker_init():
    ...
    pid=os.getpid(); _rnd.seed((pid*69069)&0xFFFFFFFF)
    _np.random_seed = (pid*1013904223)&0xFFFFFFFF; _np.random.seed(_np.random_seed)
...
class ArchiveND:
    ...
    def sample(self, k=None):
        ...
        if k is None or k >= len(self.items): return list(self.items)
...
class Optimizer(object):
    def __init__(self, multicomp_data, protocol: dict, nest_model: str, model_params: dict, opt_params: list,
                 fitness: dict, bounds,  archive=None, init_params_fn=None, evaluate_fn=None, constraints=None,
                 knee_weights = None):
        ...
        self.N_GEN = 120
        self.ETA_CROSSOVER = 10.
        self.ETA_MUTATION = 20.
        ...
        self.knee_weights = np.array(knee_weights if knee_weights is not None else np.ones(len(fitness)), dtype=float)
    ...
    def _nest_single_sim(self, current, params = None):
        with _suppress_output():
            import nest
            try: nest.set_verbosity("M_FATAL")
            except:
                try: nest.SetKernelStatus({"print_time": False})
                except: pass
            nest.ResetKernel(); nest.SetKernelStatus({"local_num_threads": 1})
            try: nest.Install("cerebmodule")
            except: pass
            nest.resolution = 0.1
        import nest
        ...
        nest.SetStatus(gen, {"amplitude": current, "start": self.protocol['start_stim'], "stop": self.protocol['end_stim']})
    ...
    def _nest_protocol(self, cell_params=None):
        ...
        return {I: self._nest_single_sim(I, params=cell_params) for I in multicomp_features['current'].values}
    ...
    def _extract_nest_features(self):
        ...
        nest_features = point_neuron_features(spike_trains=spike_trains,
    ...
    def constrained_mate(self, ind1, ind2):
        ...
        tools.cxSimulatedBinaryBounded(p1, p2, eta=self.ETA_CROSSOVER, low=lower_bounds, up=upper_bounds)
        for j in range(n):
            ind1[j] = p1[j]
            ind2[j] = p2[j]
        return ind1, ind2
    ...
    def constrained_mutate(self, ind, indpb = 0.12):
        ...
        (p_mut,) = tools.mutPolynomialBounded(p, eta=self.ETA_MUTATION,
                                              low=lower_bounds, up=upper_bounds, indpb=indpb)
        for j in range(n):
            ind[j] = p_mut[j]
        return ind,
    ...
    def pareto_selectorND(self, pop, k):
        ...
        def _evaluate_violation(ind):
            return sum(max(ind.fitness.values[i]-self.archive.tau_vec[i], 0.0) for i in range(self.archive.dim))
    ...
    def log_best(self, gen, ind, reset = False):
        import pickle
        path = getattr(self, "BEST_HYSTORY_FILE", getattr(self, "BEST_HISTORY_FILE", "best_history.pkl"))
    ...
    def plot_fI_snapshot(self, snapshots, out_png=None, use_global=False, print_params=True):
        ...
        t0, t1, T = self.protocol["start_stim"], self.protocol["end_stim"], self.protocol["duration"]

        target_freqs = neuron_data["mean_frequency"].apply(lambda v: float(np.nanmean(v)) if isinstance(v,(list,np.ndarray)) else float(v) if v is not None else 0.0).values
        ...
        for i, (gen, params) in enumerate(snapshots):
            ...
            nest_freqs = np.array(
                [_rate_for_plot(sim[I], t0, t1, T, use_global) for I in currents],
                dtype=float
            )
            is_last = (i == len(snapshots) - 1)
            plt.plot(currents, nest_freqs,
                     "s-" if is_last else "-",
    ...
    def optimize(self):
        ...
        for gen in range(self.N_GEN):
            ...
            parents_source = pop[:] + self.archive.sample(k=min(len(pop)//4, len(self.archive.items)))
            ...
            offspring = algorithms.varOr(parents, toolbox, lambda_=self.POP_SIZE,
                                         cxpb=self.CROSSOVER_PB, mutpb=self.MUTATION_PB)
            ...
            if (gen + 1) % 10 == 0:
                ...
                cell_params_best = {**self.model_params, **{k: float(v) for k, v in zip(self.opt_params, genes)}}
