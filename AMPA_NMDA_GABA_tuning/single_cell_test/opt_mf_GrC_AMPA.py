...
problem = SynapseOptimization(cell_params, sim_params, trace_file, opt_param_names, E_rev=cell_params['AMPA_E_rev'],
                              tmin=250, tmax=1000, duration=1001)
problem.xl = np.array([0, 0, 0, 0, 0, 0, 0])
problem.xu = np.array([5, 1., 1., 10., 10, 10, 10])
...
plot_pareto_front(res, sol_idx, threshold=1, title='Pareto Front for AMPA mf_Grc', save_path='./figs/Pareto_AMPA_mf_GrC.png')
...
plot_conductance(params=cell_params, sim_params=sim_params, trace_file=trace_file, E_rev=cell_params['AMPA_E_rev'],
                 optimized_params=opt_params_dict, title="AMPA conductance mf_Grc", xlim = (240,280),
                 save_path="./figs/opt_AMPA_mf_GrC.png", duration=1001)