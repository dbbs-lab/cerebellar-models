...
def single_sim(params_grc):
    syn_spec = {"weight": 1.0, "delay": 1, "receptor_type": 1}
    nest.ResetKernel()
    nest.Install("cerebmodule")
    nest.resolution = 0.025

    grc = nest.Create("eglif_multisyn", 1, params=params_grc)
    input_spikes = nest.Create("spike_generator", params={"spike_times": [250.0 - 31 * 0.025]})
...
def extract_trace():
    ...
    df["g_AMPA"] = df["Current"].values / (-40.0) * 1000
...
def objective_function(params, peak_penalty=True, w=3e-5):
    ...
    if peak_penalty:
        peak_penalty = abs(np.max(trace_g[mask_t]) - np.max(g_syn_AMPA[mask_t])) / np.max(
            trace_g[mask_t]
        )
        # print('Mse:', error)
