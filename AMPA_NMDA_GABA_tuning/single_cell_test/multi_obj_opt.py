...
def single_sim(cell_params, sim_params, E_rev, duration):
    syn_spec = {
        "weight": 1.,
        "delay": 1,
        "receptor_type": sim_params["receptor_type"]
    }
    nest.ResetKernel()
    ...
    input_spikes = nest.Create("spike_generator", params={"spike_times": [sim_params["spike_time"]]})
    ...
    spike_recorder = nest.Create("spike_recorder", params={
        "record_to": "memory",
        "label": sim_params["label_spike_recorder"]
    })
...
def extract_trace(file_path, E_rev, v_clamp=-40., mg_block = None):
...
def plot_pareto_front(res, sol_idx, threshold, title, save_path=None):
    ...
    plt.scatter(mse_vals[sol_idx], peak_penalty_vals[sol_idx] * 100,
                c='red', label='Selected solution', zorder=5, edgecolors='none')
...
def plot_conductance(params, sim_params, trace_file, E_rev,  duration, optimized_params=None, title="Synaptic conductance",
                     save_path=None, v_clamp=-40., mg_block = None,xlim=None):
...
class SynapseOptimization(Problem):
    def __init__(self, cell_params, sim_params, trace_file, param_names, E_rev, tmin, tmax ,
                 v_clamp=-40., mg_block = None, duration = 1001,
                 ):
        ...
        self.tmax = tmax
        super().__init__(
            n_var=len(param_names), n_obj=2, n_constr=1,
            xl=np.array([0, 0, 0, 0, 0, 0, 0]),
            xu=np.array([5, 5., 5., 10., 5, 5, 5])
        )
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        for params in X:
            ...
            times, g_syn = single_sim(self.cell_params, self.sim_params, self.E_rev, duration=self.duration)
            mse_val, peak_penalty = compute_metrics(times, g_syn, self.trace_g, self.tmin, self.tmax)
            f1.append(mse_val)
