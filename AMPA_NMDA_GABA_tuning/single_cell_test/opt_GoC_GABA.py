...
problem = SynapseOptimization(cell_params, sim_params, trace_file, opt_param_names, E_rev=cell_params['GABA_E_rev'],
                              duration=1201,tmin=200, tmax=1200)
problem.xl = np.array([0, 0., 0, 10, 0, 0, 0])
problem.xu = np.array([5, 5., 10., 100., 15, 10, 10])
...
plot_pareto_front(res, sol_idx, threshold=1, title='Pareto Front for GABA GoC', save_path='./figs/Pareto_GABA_GoC.png')
...
plot_conductance(params=cell_params, sim_params=sim_params, trace_file=trace_file, E_rev=cell_params['GABA_E_rev'],
                 optimized_params=opt_params_dict, title="GABA conductance GoC", xlim = (190,400), duration=1201,
